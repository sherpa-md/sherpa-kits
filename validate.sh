...
mapfile -t SHERPA_FILES < <(find kits candidates -name '*.sherpa.md' -type f | sort)
...
if grep -rnE --include='*.md' '(/home/[A-Za-z0-9_-]+|/Users/[A-Za-z0-9_-]+|[A-Za-z]:\\Users\\[A-Za-z0-9_-]+)' kits candidates >/dev/null 2>&1; then
...
if grep -rnE 'confidentiality:[[:space:]]*"private"' kits candidates catalog.json >/dev/null 2>&1; then
